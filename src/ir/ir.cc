...
void ir::Generator::init()
{
    ...
    table.insert(std::pair<std::string, std::function<llvm::Value *(std::shared_ptr<ast::Node>, ir::Block &)>>(
        ...
        [&](std::shared_ptr<ast::Node> node, ir::Block &block) -> llvm::Value * {
            ...
            llvm::Function *function = llvm::Function::Create(function_type, llvm::GlobalValue::ExternalLinkage, fun_name, Module.get());
            // set parameter name
            unsigned idx = 0;
            ...
            comp_block.basic_block = llvm::BasicBlock::Create(*Context, fun_name + "_block", function);
            ...
            std::cout << "\n[generator] function verification result: " << (function_err ? "wrong" : "correct") << std::endl;
            if (function_err)
            {
    ...
    table.insert(std::pair<std::string, std::function<llvm::Value *(std::shared_ptr<ast::Node>, ir::Block &)>>(
        ...
        [&](std::shared_ptr<ast::Node> node, ir::Block &block) -> llvm::Value * {
            ...
            Builder->CreateRet(llvm::ConstantInt::get(*Context, llvm::APInt(32, val, true)));
            return (llvm::Value *)1;
        }));
    table.insert(std::pair<std::string, std::function<llvm::Value *(std::shared_ptr<ast::Node>, ir::Block &)>>(
        "statement_list",
    ...
    std::vector<std::shared_ptr<ast::Node>> &objects)
{
    ...
    try
    {
        ...
        for (auto i = objects.begin(); i != objects.end(); ++i)
        {
            ...
            es.flush();
            std::cout << "\n[main] module verification result: " << (module_broken ? "wrong" : "correct") << std::endl;
            if (module_broken)
            {
