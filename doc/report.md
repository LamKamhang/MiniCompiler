...
- [√] Generate IR
  - [√] 基本类型: char, int(unsigned), float(double), void
  - [√] control flow
  - [√] loop
...
在我们的编译器中，生成中间代码与语义分析是结合在一起的。语义分析需要分析AST，并对有问题的节点报出错误。错误列表大致如下：

* function_definition:
  * 第一个参数为void，后续还有参数
  * 重定义函数
  * 重声明一个声明过的，但函数签名不同的函数
  * 装载实参时，类型与形参不匹配
  * 无法生成Basic Block
  * LLVM函数检验失败
...
有些错误是可以恢复的，不必直接抛出并终止语义分析的。例如：对const类型符号赋值。对于这种问题，我们的处理是const符号在声明时若没有初始化，则会自动赋一个初值。所以在后续分析过程中，但需要使用这个符号的时候，它还是有值的，不会阻碍后续语义分析。但检测到对const类型再次赋值时，会提示一个Warning。类似的还有，声明const或unsigned修饰的void类型。遇到这种情况则直接忽略const或unsigned修饰符，继续进行分析，并同样会Warning。
